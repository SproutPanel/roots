...
type DownloadModRequest struct {
	...
	Destination   string `json:"destination"` // "/mods"
...
type ModFileInfo struct {
	...
	Hash        string    `json:"hash"`
	Fingerprint uint32    `json:"fingerprint"` // CurseForge murmurhash2 fingerprint
}
...
// ListMods handles GET /api/servers/{uuid}/hytale/mods
func (h *HytaleHandlers) ListMods(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")

	_, err := h.requireHytaleServer(r.Context(), uuid)
	if err != nil {
		h.writeError(w, err)
		return
	}
	...
	for _, entry := range entries {
		...
		// Calculate CurseForge fingerprint using the official algorithm
		fingerprint := curseforgeFingerprint.GetFingerprintFor(filePath)
		h.logger.Info("Calculated CurseForge fingerprint", "file", entry.Name(), "fingerprint", fingerprint, "size", info.Size())
...
// DownloadMod handles POST /api/servers/{uuid}/hytale/mods/download
func (h *HytaleHandlers) DownloadMod(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")
	...
	// Validate destination is safe (only /mods for Hytale)
	if req.Destination != "/mods" {
		http.Error(w, "destination must be /mods", http.StatusBadRequest)
		return
	}
	...
	// Always calculate SHA512 for storage (used for update checks)
...
// DeleteMod handles DELETE /api/servers/{uuid}/hytale/mods
func (h *HytaleHandlers) DeleteMod(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")
	...
	// Security: ensure path starts with /mods
	dir := filepath.Dir(filePath)
	if dir != "/mods" {
		http.Error(w, "path must be in /mods", http.StatusBadRequest)
		return
	}
...
// ToggleMod handles POST /api/servers/{uuid}/hytale/mods/toggle
func (h *HytaleHandlers) ToggleMod(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")
	...
	// Security: ensure path is in /mods
...
// BackupMod handles POST /api/servers/{uuid}/hytale/mods/backup
func (h *HytaleHandlers) BackupMod(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")
...
// RestoreMod handles POST /api/servers/{uuid}/hytale/mods/restore
func (h *HytaleHandlers) RestoreMod(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")
	...
	for _, path := range []string{req.BackupPath, req.RestorePath} {
		...
		if dir != "/mods" && baseDir != "/mods" {
			http.Error(w, "paths must be in /mods", http.StatusBadRequest)
			return
		}
	}
