...
// ListDatapacks handles GET /api/servers/{uuid}/minecraft/datapacks
// Lists all datapacks in the world's datapacks directory
func (h *MinecraftHandlers) ListDatapacks(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("Failed to read datapacks directory", "error", err)
...
// DownloadDatapack handles POST /api/servers/{uuid}/minecraft/datapacks/download
// Downloads a datapack to the world's datapacks directory
func (h *MinecraftHandlers) DownloadDatapack(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")
	...
	if err := os.MkdirAll(datapacksDir, 0755); err != nil {
		h.logger.Error("Failed to create datapacks directory", "error", err, "path", datapacksDir)
	...
	if err != nil {
		h.logger.Error("Failed to download datapack", "error", err, "url", req.URL)
	...
	if req.ExpectedHash != "" && hash != req.ExpectedHash {
		os.Remove(destPath)
		h.logger.Error("Hash mismatch", "expected", req.ExpectedHash, "got", hash)
	...
	h.logger.Info("Datapack downloaded", "path", relativePath, "size", size)
...
// DeleteDatapack handles DELETE /api/servers/{uuid}/minecraft/datapacks
// Deletes a datapack from the world's datapacks directory
func (h *MinecraftHandlers) DeleteDatapack(w http.ResponseWriter, r *http.Request) {
	uuid := chi.URLParam(r, "uuid")
	...
	fullPath, err := h.provider.ResolvePath(serverDir, path)
	if err != nil {
	...
	if err != nil {
		h.logger.Error("Failed to delete datapack", "error", err, "path", path)
		http.Error(w, "Failed to delete datapack", http.StatusInternalServerError)
		return
	}

	h.logger.Info("Datapack deleted", "path", path)
	w.WriteHeader(http.StatusNoContent)
}

// getWorldName reads the level-name from server.properties or returns "world"
func (h *MinecraftHandlers) getWorldName(uuid string) string {
	serverDir := h.provider.GetServerDir(uuid)
